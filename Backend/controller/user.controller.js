...
// // BELOW your existing exports
// // this is for blocking user
...
//     if (!userIdToBlock) return res.status(400).json({ error: "userIdToBlock required" });
//     if (userId === userIdToBlock) return res.status(400).json({ error: "You cannot block yourself" });

//     const user = await User.findById(userId);
//     if (!user) return res.status(404).json({ error: "User not found" });

//     if (!user.blockedUsers.map(String).includes(userIdToBlock)) {
...
//   try {
//     const user = await User.findById(req.user._id).populate("blockedUsers", "fullname email avatar");
